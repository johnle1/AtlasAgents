/**
 * AES-256-GCM envelope encryption for sensitive config fields, keyed by a
 * passphrase the caller collects once at process startup.
 *
 * @remarks
 ...
 * the passphrase. Callers whose config load/save must stay synchronous
 * (many CLI/server call sites are) should resolve the passphrase into a
 * cached key exactly once, at startup, before either is called — see
 * {@link initializeCipher}/{@link unlockCipher}.
 *
 * One envelope covers all sensitive fields protected under a given
 * passphrase together (not per-field), so a salt/IV/tag triple isn't
 * duplicated per field. Every {@link encryptSecrets} call reuses the cached
 * key with a **fresh random IV** — reusing a key across many ciphertexts is
 * safe under AES-GCM as long as each encryption gets its own IV, which is
 * what makes it cheap to re-encrypt on every save without re-running
 * `scrypt` each time.
 *
 * Each caller (client CLI, server) unlocks its own independent instance of
 * this module's cached key/salt state — they run as separate processes, so
 * there is no shared runtime state between them even though they use the
 * same cipher logic.
 */
...
export class ConfigDecryptionError extends Error {
  constructor() {
    super(
      "Failed to decrypt secrets. This means either the passphrase is " +
        "wrong or the config file has been corrupted or tampered with.",
    );
...
/** Drops the cached key. Exposed for tests; normal callers never need to re-lock mid-session. */
...
const decryptWithKey = <T>(envelope: SecretsEnvelope, key: Buffer): T => {
  ...
  // past callers that specifically catch ConfigDecryptionError (e.g. a
  // wrong-passphrase retry loop).
  try {
 ...
 * stays decryptable.
 *
 * @param passphrase - The passphrase the caller just chose.
 */
 ...
 *
 * @param passphrase - The passphrase the caller just entered.
 ...
 * @example
 * ```ts
 * const secrets = rotateKey<SecretFields>("old-pass", "new-pass", envelope);
