...
describe("stripModelThinkingBlocks", () => {
  it("removes agent-think and redacted reasoning blocks", () => {
    const raw = `<agent-think>hidden</agent-think>visible
<redacted_reasoning>also hidden</redacted_reasoning> tail`;

    expect(stripModelThinkingBlocks(raw)).toBe("visible\n tail");
  });
});

describe("defaultAgentFields", () => {
  it("fills missing agentId and agentLabel", () => {
    const normalized = defaultAgentFields([
      {
        id: 1,
        text: "work",
        dependsOn: [],
    ...
    expect(normalized[0]?.agentId).toBe(1);
    expect(normalized[0]?.agentLabel).toBe("tasks");
  });
});

describe("buildPlanRevisionTask", () => {
  it("embeds user feedback for a replanning round", () => {
    const revised = buildPlanRevisionTask("Build login", "Use hooks");
    expect(revised).toContain("Build login");
    expect(revised).toContain("Use hooks");
    expect(revised).toContain("Revise the plan");
  });
});
