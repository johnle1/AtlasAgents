...
describe("buildTaskBoardLines — normal cases", () => {
  ...
  it("does not append activity lines on the task board (normal)", () => {
    ...
    expect(lines.map((l) => l.text).join(" ")).not.toContain("Reading src/App.tsx");
  });
  ...
  it("suppresses run_command activity lines (Running: …)", () => {
    ...
    expect(lines.some((l) => l.isActivity)).toBe(false);
    expect(lines.some((l) => l.text.includes("Running:"))).toBe(false);
  });
});
