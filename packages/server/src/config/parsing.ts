...
/**
 * Ollama duration literal, per Go's `time.ParseDuration` — which is what
 * Ollama runs a string `keep_alive` through. A bare `-1` has no unit and is
 * rejected there, so never-unload has to travel as the JSON number instead.
 *
 * @remarks
 * `time.ParseDuration` accepts one or more consecutive `<number><unit>`
 * segments with no separator (e.g. `"1h30m"`, `"2h45m30s"`), not just a
 * single segment — matching only `/^\d+(\.\d+)?(unit)$/` would reject every
 * valid compound duration.
 */
const OLLAMA_DURATION_SEGMENT = String.raw`\d+(?:\.\d+)?(?:ns|us|µs|ms|s|m|h)`;
const OLLAMA_DURATION_PATTERN = new RegExp(
  `^(?:${OLLAMA_DURATION_SEGMENT})+$`,
);

/**
 * Validates and normalizes a `keepAlive` value into a form Ollama accepts.
 *
 * @remarks
 * Shared by `ConfigManager.set` (so a bad value is rejected at write time)
 * and by {@link mergeStoredConfig} (so a value persisted before this
 * validation existed is repaired on load rather than breaking every request).
 *
 * The string `"-1"` is normalized to the number `-1`. It reads as the obvious
 * spelling of never-unload — the type's own docs used to suggest it — but
 * Ollama would 400 on it, taking down every `/api/chat` and `/api/generate`
 * call until the config file was hand-edited.
 *
 * @param value - Raw value from a `set()` call or from disk.
 * @returns The normalized value, or `null` if it is not a usable `keep_alive`.
 *
 * @example
 * ```ts
 * normaliseKeepAlive("30m");   // "30m"
 * normaliseKeepAlive("-1");    // -1   (normalized to the number)
 * normaliseKeepAlive(-1);      // -1
 * normaliseKeepAlive("forever"); // null
 * ```
 */
export const normaliseKeepAlive = (value: unknown): string | number | null => {
  if (typeof value === "number") {
    // -1 is never-unload; any other finite value is a second count.
  ...
  return OLLAMA_DURATION_PATTERN.test(trimmed) ? trimmed : null;
};
...
/**
 * Coerces an unknown value to a positive integer, or `undefined` if it isn't
 * one.
 *
 * @remarks
 * Mirrors `ConfigManager.set`'s own `numCtx` validation (positive integer
 * token count), so a value that predates that validation — or was edited by
 * hand into the config file — is repaired on load rather than surviving as
 * a negative, fractional, or zero `num_ctx` sent straight to Ollama.
 *
 * @param value - Raw value from disk.
 * @returns `value` if it's a positive integer, otherwise `undefined`.
 *
 * @example
 * ```ts
 * asPositiveInteger(4096);    // 4096
 * asPositiveInteger(-100);    // undefined
 * asPositiveInteger(0.5);     // undefined
 * asPositiveInteger("4096");  // undefined
 * ```
 */
...
export const mergeConfig = (storedConfig: Record<string, unknown>): ServerConfig => {
  ...
  return {
    ...
    // Ollama runtime context window: no default — undefined means "let
    // Ollama use its own default" rather than guessing a number. Repaired
    // the same way as keepAlive below: `set()` requires a positive integer,
    // but a value written before that validation existed (or edited by
    // hand) could be negative, fractional, or zero — asFinite-only checks
