 ...
 * import { SubagentStatusBox } from "./SubagentStatusBox.js";
 * 
 * render(
 *   <SubagentStatusBox
 *     id={2}
 ...
 * ```
 */
export const SubagentStatusBox: React.FC<Props> = ({
  id,
  label,
  icon,
  message,
  ...
  // Agent has unique styling and labels distinct from numerical subagents.
  const isAgent = id === "agent";

  // Maps high-level stages and status icons to terminal formatting (color, dim, character glyph).
  const visual = resolveWorkerVisual(stage, icon, pulseIndex, isAgent);
  ...
  // Format a friendly display title. Agent gets a static name, subagents show ID and custom label.
  const title = isAgent
    ? "Agent"
    : label.length > 0
      ? `Subagent ${id} — ${label}`
