...
describe("Agent.plan without native tool support", () => {
  ...
  it("stops planning when the caller's signal is already aborted, instead of ignoring it (regression guard)", async () => {
    // plan() previously had no signal parameter at all — the caller's abort
    // was never forwarded to chatStream/chatWithTools, so a disconnected
    // client's planning kept running: inference continued and think/status
    // frames kept trying to emit to a socket that no longer existed.
    let chatStreamCalled = false;
    const ollama = {
    ...
    // The abort must be checked before ever starting the model call, not
    // just passed through as an ignored option.
    expect(chatStreamCalled).toBe(false);
  });

  it("stops mid-stream once the signal aborts partway through chatStream (regression guard)", async () => {
    const controller = new AbortController();
    const ollama = {
      chatStream: async function* (): AsyncGenerator<string> {
        yield VALID_THINK_BLOCK.slice(0, 20);
        controller.abort(); // client disconnects mid-turn
        yield VALID_THINK_BLOCK.slice(20);
        yield `\n${JSON.stringify(VALID_PLAN_ARGS)}`;
      },
    } as unknown as IOllamaClient;
...
describe("Agent.plan tool-name guards (isExploreCodebaseTool / isAgentSearchToolName)", () => {
  it("routes explore_codebase tool calls to the explore hook", async () => {
    let exploreInvoked = false;
    let chatCallCount = 0;
    const ollama = {
      chatWithTools: async () => {
        ...
        if (chatCallCount === 1) {
          return {
            content: VALID_THINK_BLOCK,
            toolCalls: [
              { name: EXPLORE_CODEBASE_TOOL_NAME, args: { reason: "map" } },
            ],
          };
        }
        return {
    ...
    const plan = await createAgent(ollama).plan("task", "", "", undefined, {
      exploreCodebase: async () => {
        exploreInvoked = true;
        return { snapshot: "Structure:\nx\n" };
      },
      reviewPlan: async () => ({ decision: "implement" }),
    });
  ...
  it("routes tokensave_* search tools to callSearchTool", async () => {
    let searchInvoked = false;
    let chatCallCount = 0;
    const ollama = {
      chatWithTools: async () => {
        ...
        if (chatCallCount === 1) {
          return {
            content: VALID_THINK_BLOCK,
            toolCalls: [{ name: "tokensave_search", args: { query: "auth" } }],
          };
        }
        return {
          content: VALID_THINK_BLOCK,
...
describe("Agent.plan pushThink hook wiring", () => {
  it("forwards incremental think deltas through onThinkDelta", async () => {
    const deltas: string[] = [];
    const ollama = {
      chatWithTools: async (
        _model: string,
    ...
    await createAgent(ollama).plan("task", "", "", undefined, {
