...
import { ConfigManager } from "../../../packages/server/src/config/configManager/index.js";
import {
  ConfigDecryptionError,
  lockCipher,
} from "../../../packages/server/src/crypto/configCipher.js";

const tempRoots: string[] = [];

const makeManager = async (): Promise<{ manager: ConfigManager; root: string }> => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "loopy-providers-cipher-"));
  tempRoots.push(root);
...
describe("ConfigManager.unlockOrSetupProvidersCipher — first run", () => {
  ...
  it("encrypts providers on the first addProvider call", async () => {
    ...
    await manager.addProvider("openai", { baseUrl: "https://api.openai.com", apiKey: "sk-secret" });

    const raw = await fs.readFile(path.join(root, "user-data", "config.json"), "utf-8");
...
describe("ConfigManager.unlockOrSetupProvidersCipher — restart", () => {
  ...
  it("forgot passphrase: 'reset' + confirmed wipes provider secrets but preserves everything else", async () => {
    const { manager, root } = await makeManager();
    await manager.unlockOrSetupProvidersCipher(async () => "correct-pass");
    await manager.addProvider("vllm", { baseUrl: "http://10.0.0.9:8000", apiKey: "sk-old" });
    ...
    expect(await fs.readFile(path.join(path.dirname(configPath), backups[0]), "utf-8")).toBe(
      before,
    );
...
describe("ConfigManager.rotateProvidersPassphrase", () => {
  it("re-encrypts existing provider secrets under a new passphrase, preserving values", async () => {
    ...
    expect(await manager.getProviders()).toEqual({
      openai: { baseUrl: "https://api.openai.com", apiKey: "sk-before-rotation" },
    });
  ...
  it("the new passphrase unlocks it correctly after a simulated restart", async () => {
    const { manager, root } = await makeManager();
    await manager.unlockOrSetupProvidersCipher(async () => "old-pass");
    await manager.addProvider("vllm", { baseUrl: "http://10.0.0.9:8000", apiKey: "sk-vllm" });
  ...
  it("throws ConfigDecryptionError and changes nothing when currentPassphrase is wrong", async () => {
    const { manager, root } = await makeManager();
    await manager.unlockOrSetupProvidersCipher(async () => "correct-pass");
    await manager.addProvider("openai", { baseUrl: "https://api.openai.com", apiKey: "sk-unchanged" });
    const configPath = path.join(root, "user-data", "config.json");
    const before = await fs.readFile(configPath, "utf-8");
...
describe("ConfigManager.unlockOrSetupProvidersCipher — legacy plaintext migration", () => {
  it("migrates a plaintext providers map in place, preserving values, with a backup", async () => {
    const { manager, root } = await makeManager();
    const configPath = path.join(root, "user-data", "config.json");
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        providers: {
          openai: { baseUrl: "https://api.openai.com", apiKey: "plaintext-legacy-key" },
        },
      }),
    );

    await manager.unlockOrSetupProvidersCipher(async () => "migration-pass");
    ...
    expect(raw).toContain("$providersSecrets");
    expect(raw).not.toContain("plaintext-legacy-key");

    const files = await fs.readdir(path.dirname(configPath));
    const backups = files.filter((name) => name.startsWith("config.json.bak-"));
    expect(backups).toHaveLength(1);
    const backupContent = await fs.readFile(path.join(path.dirname(configPath), backups[0]), "utf-8");
    expect(backupContent).toContain("plaintext-legacy-key");
  });

  it("skips the migration write when there are no providers to protect", async () => {
    const { manager, root } = await makeManager();
    const configPath = path.join(root, "user-data", "config.json");
    await fs.mkdir(path.dirname(configPath), { recursive: true });
    await fs.writeFile(configPath, JSON.stringify({ agentModel: "gemma3:27b" }));

    await manager.unlockOrSetupProvidersCipher(async () => "no-providers-pass");

    // No backup should have been made — nothing sensitive was on disk.
    const files = await fs.readdir(path.dirname(configPath));
    expect(files.some((name) => name.startsWith("config.json.bak-"))).toBe(false);
    // Cipher is still usable for subsequent writes.
    await manager.addProvider("openai", { baseUrl: "https://api.openai.com" });
    expect(await manager.getProviders()).toEqual({
