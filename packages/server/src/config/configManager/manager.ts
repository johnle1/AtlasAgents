...
import { parseStoredConfig, mergeConfig, WRITABLE_CONFIG_KEYS } from "./parsing.js";
...
export class ConfigManager implements IConfigManager {
  ...
  private _saveRaw = async (config: ServerConfig): Promise<void> => {
    ...
    // single envelope rather than writing them as plaintext JSON.
    const { providers, ...rest } = config;
    const providersSecrets = encryptSecrets(providers);
    const onDisk = { ...rest, $providersSecrets: providersSecrets };
  ...
  private offerProvidersPassphraseReset = async (
    ...
    const { $providersSecrets: _oldSecrets, providers: _oldProviders, ...rest } = stored;
  ...
  unlockOrSetupProvidersCipher = async (
    ...
    if (stored.$providersSecrets) {
      ...
      for (;;) {
        ...
        try {
          ...
          if (
            (await this.offerProvidersPassphraseReset(promptPassphrase, stored)) ===
            "unlocked"
          ) {
            return;
          }
    ...
    initializeCipher(await promptNewPassphrase());
    if (!stored.providers || Object.keys(stored.providers as object).length === 0) {
