...
import type { Session, RouterBuilderDeps } from "../../packages/server/src/routing/types.js";
...
import { lockCipher } from "../../packages/server/src/crypto/configCipher.js";
...
const makeConfigManager = async (): Promise<{ config: ConfigManager; root: string }> => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "loopy-router-command-flow-"));
  tempRoots.push(root);
...
const makeRouter = (config: ConfigManager, ollama: OllamaClient = new OllamaClient()) => {
...
describe("config.set — value validation propagates through the route", () => {
  it("accepts an in-range agentTemp", async () => {
    ...
    await router.routeCommand(SESSION, "config.set", { key: "agentTemp", value: 0.2 });
  ...
  it("rejects an out-of-range agentTemp rather than silently clamping it", async () => {
    ...
    await expect(
      router.routeCommand(SESSION, "config.set", { key: "agentTemp", value: 1.5 }),
...
describe("models.delete — reports the correct role flags (no live Ollama)", () => {
  it("flags wasAgentModel when the deleted model matches the agent role", async () => {
    ...
    expect(result).toEqual({ ok: true, wasAgentModel: true, wasSubagentModel: false });
  });
  ...
  it("flags wasSubagentModel when the deleted model matches the subagent role", async () => {
    ...
    expect(result).toEqual({ ok: true, wasAgentModel: false, wasSubagentModel: true });
  });
  ...
  it("flags neither when the deleted model matches no configured role", async () => {
    ...
    expect(result).toEqual({ ok: true, wasAgentModel: false, wasSubagentModel: false });
  });
});
