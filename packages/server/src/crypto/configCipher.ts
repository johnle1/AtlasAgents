/**
 * AES-256-GCM envelope encryption for the server's `providers` config field
 * (third-party model provider `baseUrl`/`apiKey` pairs), keyed by a
 * passphrase the operator enters once at server startup.
 *
 * @remarks
 * This is a separate cipher/passphrase from the client's
 * (`packages/client/src/crypto/configCipher.ts`) — they protect different
 * secrets, on different machines, with different lifetimes. Deriving one
 * passphrase from the other would mean rotating the CLI's auth password
 * also orphans the server's encrypted provider keys, so they're kept
 * independent even though the cipher logic is identical.
 *
 * The key is derived with `scrypt` (deliberately slow/memory-hard, so
 * brute-forcing a stolen config file is expensive) and held in memory only
 * for the life of the process — it is never written to disk, and neither is
 * the passphrase. The passphrase must be resolved into this cached key once
 * at server startup, before `configManager.ts`'s `_loadRaw`/`_saveRaw` are
 * first called — see {@link initializeCipher}/{@link unlockCipher}.
 *
 * One envelope covers the whole `providers` map together (not per-provider),
 * so a salt/IV/tag triple isn't duplicated per provider. Every
 * {@link encryptSecrets} call reuses the cached key with a **fresh random
 * IV** — reusing a key across many ciphertexts is safe under AES-GCM as long
 * as each encryption gets its own IV, which is what makes it cheap to
 * re-encrypt on every save without re-running `scrypt` each time.
 */
...
export class ConfigDecryptionError extends Error {
  constructor() {
    super(
      "Failed to decrypt provider secrets. This means either the passphrase " +
...
/** Drops the cached key. Exposed for tests; the server never needs to re-lock mid-session. */
...
const decryptWithKey = <T>(envelope: SecretsEnvelope, key: Buffer): T => {
  ...
  // wrong-passphrase retry loop in configManager.ts's
  // unlockOrSetupProvidersCipher).
  try {
 ...
 * stays decryptable.
 *
 * @param passphrase - The passphrase the operator just chose.
 */
 ...
 *
 * @param passphrase - The passphrase the operator just entered.
 * @param envelope - The `$providersSecrets` envelope currently on disk,
 *   whose `salt` is reused so the derived key matches the one that
 *   encrypted it.
 ...
 * @example
 * ```ts
 * const providers = rotateKey<Record<string, ProviderConfig>>("old-pass", "new-pass", envelope);
 * const rotatedEnvelope = encryptSecrets(providers); // now under the new key
 * ```
 */
