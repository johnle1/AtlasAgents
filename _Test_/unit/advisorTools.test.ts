/**
 * Unit tests — server orchestration/advisor/advisorTools.ts
 */

import { describe, expect, it } from "vitest";
import { ADVISOR_PLAN_TOOL_SCHEMA } from "../../packages/server/src/orchestration/advisor/advisorThink.js";
import {
  ADVISOR_EXPLORATION_RULES,
  ADVISOR_RETRIEVAL_RULES,
  EXPLORE_CODEBASE_TOOL,
  EXPLORE_CODEBASE_TOOL_NAME,
  hasAdvisorSearchTools,
  isAdvisorPrePlanToolName,
  isAdvisorSearchToolName,
...
describe("hasAdvisorSearchTools", () => {
  it("returns false for empty array", () => {
    expect(hasAdvisorSearchTools([])).toBe(false);
  });
  ...
  it("returns false when no tokensave_* tools", () => {
    expect(hasAdvisorSearchTools([tokensaveTool("read_file")])).toBe(false);
  });
...
describe("ADVISOR_RETRIEVAL_RULES", () => {
  const expectedTools = [
    "tokensave_search",
    "tokensave_context",
    "tokensave_callers",
  ...
  for (const tool of expectedTools) {
    it(`mentions ${tool}`, () => {
      expect(ADVISOR_RETRIEVAL_RULES).toContain(tool);
    });
  }

  it("mentions submit_plan guidance", () => {
    expect(ADVISOR_RETRIEVAL_RULES).toContain("submit_plan");
  });
});

describe("ADVISOR_EXPLORATION_RULES", () => {
  it("mentions explore_codebase and EXPLORATION", () => {
    expect(ADVISOR_EXPLORATION_RULES).toContain("explore_codebase");
    expect(ADVISOR_EXPLORATION_RULES).toContain("EXPLORATION");
  });
});
...
describe("explore codebase tool helpers", () => {
  ...
  it("identifies tokensave search tool names", () => {
    expect(isAdvisorSearchToolName("tokensave_search")).toBe(true);
    expect(isAdvisorSearchToolName("explore_codebase")).toBe(false);
  });
  ...
  it("groups pre-plan tools", () => {
    expect(isAdvisorPrePlanToolName("explore_codebase")).toBe(true);
    expect(isAdvisorPrePlanToolName("tokensave_context")).toBe(true);
    expect(isAdvisorPrePlanToolName("submit_plan")).toBe(false);
  });
});
...
describe("SUBMIT_PLAN_TOOL / ADVISOR_PLAN_TOOL_SCHEMA", () => {
  it("exports submit_plan schema from advisorThink", () => {
    expect(ADVISOR_PLAN_TOOL_SCHEMA.function.name).toBe("submit_plan");
