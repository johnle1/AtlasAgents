...
import {
  ConfigError,
  ConfigManager,
} from "../../../packages/server/src/config/configManager/index.js";
import { initializeCipher } from "../../../packages/server/src/crypto/configCipher.js";
...
describe("ConfigManager provider defaults — back-compat", () => {
  ...
  it("defaults to 'ollama' when an existing config.json predates the provider fields", async () => {
    ...
    await fs.writeFile(
      configPath,
      JSON.stringify({ subagentModel: "gemma3:27b", subsubagentModel: "gemma3:4b" }),
    );
...
describe("ConfigManager.addProvider / getProvider / getProviders", () => {
  it("adds a provider and makes it retrievable", async () => {
    const { manager } = await makeManager();
    await manager.addProvider("vllm-gpu", { baseUrl: "http://localhost:8000/v1" });
  ...
  it("upserts (overwrites) an existing provider entry", async () => {
    const { manager } = await makeManager();
    await manager.addProvider("vllm-gpu", { baseUrl: "http://localhost:8000/v1" });
    await manager.addProvider("vllm-gpu", { baseUrl: "http://localhost:9000/v1" });
...
describe("ConfigManager.removeProvider", () => {
  ...
  it("rejects removing a provider currently in use by a role", async () => {
    ...
    await expect(manager.removeProvider("vllm-gpu")).rejects.toThrow(ConfigError);
  });
});
...
describe("ConfigManager.setProvider", () => {
  ...
  it("rejects switching to an unconfigured provider", async () => {
    const { manager } = await makeManager();
    await expect(manager.setProvider("agent", "does-not-exist")).rejects.toThrow(
      ConfigError,
    );
  });
...
describe("ConfigManager.setRoleModel", () => {
  ...
  it("triggers onModelChanged with the prior model on each real change", async () => {
    ...
    manager.setOnModelChanged((oldModel, role) => changes.push([oldModel, role]));
